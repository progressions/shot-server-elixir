defmodule Mix.Tasks.ExportWeaponImages do
  ...
  def run(_args) do
    ...
    attachments = Repo.all(
      ...
      join: b in ShotElixir.ActiveStorage.Blob, on: a.blob_id == b.id,
    ...
    if length(attachments) == 0 do
      IO.puts("-- WARNING: No weapon images found! Make sure weapons have images uploaded in this environment.")
    else
      ...
      Enum.each(attachments, fn {_attachment, blob} ->
        ...
        IO.puts("INSERT INTO active_storage_blobs (id, key, filename, content_type, metadata, service_name, byte_size, checksum, created_at) VALUES (#{blob.id}, '#{key}', '#{filename}', '#{content_type}', '#{metadata}', '#{service_name}', #{blob.byte_size || 0}, '#{checksum}', '#{format_timestamp(blob.created_at)}') ON CONFLICT (id) DO NOTHING;")
      end)
      ...
      Enum.each(attachments, fn {attachment, _blob} ->
        ...
        IO.puts("INSERT INTO active_storage_attachments (id, name, record_type, record_id, blob_id, created_at) VALUES (#{attachment.id}, '#{name}', '#{record_type}', '#{attachment.record_id}', #{attachment.blob_id}, '#{format_timestamp(attachment.created_at)}') ON CONFLICT (id) DO NOTHING;")
      end)
    end
