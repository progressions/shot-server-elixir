defmodule ShotElixir.Characters do
  ...
  def list_campaign_characters(campaign_id, params \\ %{}, current_user \\ nil) do
    ...
    query = if params["party_id"] do
      from c in query,
        join: m in "memberships", on: m.character_id == c.id,
    ...
    query = if params["fight_id"] do
      from c in query,
        join: s in "shots", on: s.character_id == c.id,
        where: s.fight_id == ^params["fight_id"]
    else
      query
    end

    query = if params["site_id"] do
      from c in query,
        join: a in "attunements", on: a.character_id == c.id,
