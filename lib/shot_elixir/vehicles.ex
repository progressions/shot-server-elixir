defmodule ShotElixir.Vehicles do
  ...
  def list_campaign_vehicles(campaign_id, params \\ %{}, _current_user \\ nil) do
    ...
    query = if params["show_hidden"] == "true" do
      query
    else
    ...
    # Juncture filtering
    query = if params["juncture_id"] do
    ...
    query = if params["vehicle_type"] do
      from v in query, where: fragment("?->>'Type' = ?", v.action_values, ^params["vehicle_type"])
    else
      query
    end

    # Archetype filtering
    query = if params["archetype"] do
      if params["archetype"] == "__NONE__" do
        from v in query, where: fragment("?->>'Archetype' = ''", v.action_values) or fragment("?->>'Archetype' IS NULL", v.action_values)
      else
        from v in query, where: fragment("?->>'Archetype' = ?", v.action_values, ^params["archetype"])
      end
    else
      query
    end
    ...
    # Party filtering
    query = if params["party_id"] do
      from v in query,
        join: pm in "party_memberships", on: pm.vehicle_id == v.id,
    ...
    # Fight filtering
    query = if params["fight_id"] do
      from v in query,
        join: s in "shots", on: s.vehicle_id == v.id,
    ...
    count_query = from v in Vehicle,
      where: v.campaign_id == ^campaign_id

    # Apply same filters to count query
    count_query = if params["show_hidden"] == "true" do
      count_query
    else
    ...
    count_query = if params["faction_id"] do
      if params["faction_id"] == "__NONE__" do
    ...
    count_query = if params["juncture_id"] do
      if params["juncture_id"] == "__NONE__" do
    ...
    count_query = if params["user_id"] do
      from v in count_query, where: v.user_id == ^params["user_id"]
    else
      count_query
    end

    count_query = if params["vehicle_type"] do
      from v in count_query, where: fragment("?->>'Type' = ?", v.action_values, ^params["vehicle_type"])
    else
      count_query
    end

    count_query = if params["archetype"] do
      if params["archetype"] == "__NONE__" do
        from v in count_query, where: fragment("?->>'Archetype' = ''", v.action_values) or fragment("?->>'Archetype' IS NULL", v.action_values)
      else
        from v in count_query, where: fragment("?->>'Archetype' = ?", v.action_values, ^params["archetype"])
      end
    else
      count_query
    end

    count_query = if params["party_id"] do
    ...
    count_query = if params["fight_id"] do
      from v in count_query,
        join: s in "shots", on: s.vehicle_id == v.id,
        where: s.fight_id == ^params["fight_id"]
    else
    ...
    factions_query = from v in Vehicle,
      where: v.campaign_id == ^campaign_id and not is_nil(v.faction_id),
      join: f in "factions", on: f.id == v.faction_id,
    ...
    vehicles_for_meta = Repo.all(from v in Vehicle, where: v.campaign_id == ^campaign_id, select: v.action_values)
    archetypes = vehicles_for_meta
  ...
  def update_chase_state(%Vehicle{} = vehicle, chase_params) do
    ...
    with :ok <- validate_chase_params(chase_params) do
      updated_values = Map.merge(vehicle.action_values || %{}, build_chase_state_updates(chase_params))
  ...
  defp validate_chase_params(params) do
    ...
    errors = if params["position"] && params["position"] not in ["near", "far"] do
      ["Position must be 'near' or 'far'" | errors]
    else
      errors
    end

    errors = if params["chase_points"] && String.to_integer(params["chase_points"]) < 0 do
      ["Chase points cannot be negative" | errors]
    else
      errors
    end

    errors = if params["condition_points"] && String.to_integer(params["condition_points"]) < 0 do
