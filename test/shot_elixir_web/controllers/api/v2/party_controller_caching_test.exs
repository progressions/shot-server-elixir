defmodule ShotElixirWeb.Api.V2.PartyControllerCachingTest do
  @moduledoc """
  Tests for HTTP caching behavior in the Party controller.

  Tests ETag-based conditional requests and Cache-Control headers.
  """
  use ShotElixirWeb.ConnCase, async: true

  alias ShotElixir.{Parties, Campaigns, Accounts}
  alias ShotElixir.Guardian

  setup %{conn: conn} do
    {:ok, gamemaster} =
      Accounts.create_user(%{
        email: "party_caching_gm@example.com",
    ...
    {:ok, campaign} =
      Campaigns.create_campaign(%{
        name: "Party Caching Test Campaign",
        description: "Campaign for party caching tests",
  ...
  describe "show caching" do
    ...
    test "returns 304 Not Modified when If-None-Match matches ETag", %{
      ...
      conn1 = get(conn1, ~p"/api/v2/parties/#{party.id}")
    ...
    test "returns 200 when If-None-Match does not match current ETag", %{
      ...
      assert response["id"] == party.id
    end

    test "returns new ETag after party is updated", %{
      conn: conn,
      gamemaster: gm,
      party: party
    } do
      ...
      # Update the party - wait a moment to ensure updated_at changes
