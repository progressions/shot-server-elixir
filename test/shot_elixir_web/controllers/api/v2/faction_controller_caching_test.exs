defmodule ShotElixirWeb.Api.V2.FactionControllerCachingTest do
  @moduledoc """
  Tests for HTTP caching behavior in the Faction controller.

  Tests ETag-based conditional requests and Cache-Control headers.
  """
  use ShotElixirWeb.ConnCase, async: true

  alias ShotElixir.{Factions, Campaigns, Accounts}
  alias ShotElixir.Guardian

  setup %{conn: conn} do
    {:ok, gamemaster} =
      Accounts.create_user(%{
        email: "faction_caching_gm@example.com",
    ...
    {:ok, campaign} =
      Campaigns.create_campaign(%{
        name: "Faction Caching Test Campaign",
        description: "Campaign for faction caching tests",
  ...
  describe "show caching" do
    ...
    test "returns 304 Not Modified when If-None-Match matches ETag", %{
      ...
      conn1 = get(conn1, ~p"/api/v2/factions/#{faction.id}")
      ...
      conn2 =
        ...
        |> get(~p"/api/v2/factions/#{faction.id}")
    ...
    test "returns new ETag after faction is updated", %{
      conn: conn,
      gamemaster: gm,
      faction: faction
    } do
      ...
      # Update the faction - wait a moment to ensure updated_at changes
