defmodule ShotElixirWeb.Api.V2.CharacterControllerCachingTest do
  @moduledoc """
  Tests for HTTP caching behavior in the Character controller.
  ...
  setup %{conn: conn} do
    ...
    {:ok, campaign} =
      Campaigns.create_campaign(%{
        name: "Caching Test Campaign",
        description: "Campaign for caching tests",
  ...
  describe "show caching" do
    ...
    test "returns 304 Not Modified when If-None-Match matches ETag", %{
      ...
      conn1 = get(conn1, ~p"/api/v2/characters/#{character.id}")
    ...
    test "returns new ETag after character is updated", %{
      conn: conn,
      gamemaster: gm,
      character: character
    } do
      ...
      # Update the character - wait a moment to ensure updated_at changes
